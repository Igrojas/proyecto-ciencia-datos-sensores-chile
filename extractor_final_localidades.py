#!/usr/bin/env python3
"""
Script final corregido para obtener datos de calidad del aire de localidades específicas de Chile:
- Bocatoma
- ENAP Price
- JUNJI
- Indura
                    ...
                    if (hasattr(location, 'country') and location.country and 
                        ...
                        for localidad_buscar in localidades_buscar:
                            if localidad_buscar.lower() in nombre_ubicacion:
                                ...
                                print(f"    ✓ Encontrada '{localidad_buscar}' en: {location.name}")
...
def obtener_mediciones_maximas(client, sensores, max_mediciones_por_sensor=1000):
    """
    Obtener la máxima cantidad de mediciones posible de cada sensor (máximo 1000 por API)
    """
    ...
    for i, sensor in enumerate(sensores):
        ...
        try:
            # Intentar obtener el máximo de mediciones (máximo 1000 por API)
...
def convertir_a_dataframe(mediciones):
    ...
    for medicion in mediciones:
        try:
            datos_dict = {
                'parametro_nombre': medicion.parameter.name if hasattr(medicion, 'parameter') and medicion.parameter else None,
                'parametro_id': medicion.parameter.id if hasattr(medicion, 'parameter') and medicion.parameter else None,
                'valor': medicion.value if hasattr(medicion, 'value') else None,
                'unidad': medicion.parameter.units if hasattr(medicion, 'parameter') and medicion.parameter else None,
                'fecha_desde_utc': medicion.period.datetime_from.utc if hasattr(medicion, 'period') and medicion.period else None,
                'fecha_desde_local': medicion.period.datetime_from.local if hasattr(medicion, 'period') and medicion.period else None,
                'fecha_hasta_utc': medicion.period.datetime_to.utc if hasattr(medicion, 'period') and medicion.period else None,
                'fecha_hasta_local': medicion.period.datetime_to.local if hasattr(medicion, 'period') and medicion.period else None,
                'cobertura_porcentaje': medicion.coverage.percent_complete if hasattr(medicion, 'coverage') and medicion.coverage else None,
            ...
            print(f"  ⚠ Error al procesar medición: {e}")
            continue
    
    if not datos_lista:
        print("⚠ No se pudieron procesar las mediciones")
        return None
    ...
    if 'parametro_nombre' in df.columns:
        print(f"  - Parámetros disponibles: {', '.join(df['parametro_nombre'].dropna().unique())}")
    if 'localidad_buscada' in df.columns:
        print(f"  - Localidades encontradas: {', '.join(df['localidad_buscada'].dropna().unique())}")
    if 'fecha_desde_utc' in df.columns:
...
def main():
    ...
    print("=== EXTRACTOR FINAL DE DATOS DE CALIDAD DEL AIRE - LOCALIDADES ESPECÍFICAS DE CHILE ===\n")
